...
from app.fetcher.function import target_fetch, get_time, get_html_by_requests, get_random_user_agent
from rules import RULES, LATEST_RULES
...
def cache_owllook_novels_chapter(url, netloc):
    ...
    html = target_fetch(headers=headers, url=url)
    if not html:
        html = get_html_by_requests(url=url, headers=headers)
    if html:
        soup = BeautifulSoup(html, 'html5lib')
...
def cache_owllook_search_ranking():
...
def cache_others_search_ranking(spider='qidian', novel_type='全部类别'):
    motor_db = MotorBase().get_db()
    ...
    try:
        ...
        if url and novels_name:
            ...
            if netloc in LATEST_RULES.keys():
                ...
                try:
                    html = target_fetch(url=url, headers=headers, timeout=timeout)
                    if html is None:
...
def update_all_books(loop, timeout=15):
    try:
        motor_db = MotorBase().get_db()
        # 获取所有书架链接游标
